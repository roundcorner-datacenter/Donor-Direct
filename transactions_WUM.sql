...
-- Drop staging tables
drop table [Migration_WUM].dbo.Opportunity_Transactions
--Replicate Data
...
,Concat(a.Name,' ','DONATION',' ',LEFT(CONVERT(VARCHAR, rtrim(ltrim(xx.START_DT)), 120),4))+'PAYMENT'+Left(rtrim(ltrim(xx.[DATE])),10) As Name
...
,'WUM-TRANSACTION-'+rtrim(ltrim(xx.GIDNUMB))							As External_id__c
...
,CAST('' as nvarchar(255))												As rC_Giving__Payment_Count__c
...
INTO [Migration_WUM].dbo.[Opportunity_Transactions] 
FROM
[WUM_UAT1].dbo.dpgift
xx
Left join
...
---------------------------------------------------------------------------------------------------------------------------------
-- update  [Migration_WUM].dbo.[Opportunity_Transactions] 
...
left join
[Migration_WUM].dbo.[Opportunity_Transactions] oi
...
-- FROM 
  -- [Migration_WUM].dbo.[Opportunity_Transactions] a
-- Left join
-- (Select gidnumb, glink,sol,donor,amt from [WUM_UAT1].dbo.dpgift 
-- where glink != gidnumb and glink != '0' )xx
-- on xx.gidnumb=substring(a.[External_id__c],17,Len(a.[External_id__c])) 
-- Left join
...
-- on substring(a.[External_id__c],17,Len(a.[External_id__c])) =
...
-- Left join
 -- [Migration_WUM].dbo.[Opportunity_Transactions] a
-- on xx.gidnumb=substring([External_id__c],17,Len([External_id__c]))
--------------
...
on xx.gidnumb=substring([External_id__c],17,Len([External_id__c]))
-------------------------------------------------------------------------------------------
----updating rC_Giving__Parent__c

Update [Migration_WUM].dbo.[Opportunity_Transactions]
SET rC_Giving__Parent__c= od.ID from [Migration_WUM].dbo.[opportunity_DONATION] od
join [Migration_WUM].dbo.[Opportunity_Transactions] ot
on SUBSTRING(od.External_ID__c,14,len(od.External_ID__c)-13)
=SUBSTRING(ot.External_ID__c,17,len(ot.External_ID__c)-16)
...
-- use [Migration_WUM]
--exec SF_BulkOps 'Insert:bulkapi,Parallel,batchsize(10000)','TSAC_MIGR','Opportunity_Transactions'
