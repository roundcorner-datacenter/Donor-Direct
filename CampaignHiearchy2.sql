...
DROP TABLE [Migration_Metro].dbo.[Campaign_Load_Hiearchy2]
...
where NAME='Standard' and [SobjectType]='campaign') As RecordTypeId,
 xx.Year_Code,
 xx.campaign
INTO [Migration_Metro].dbo.[Campaign_Load_Hiearchy2]
 from
...
union
SELECT distinct ltrim(rtrim(a.campaign)) campaign, ltrim(rtrim(b.descr)) descr, substring(a.code, 3,2) As Year_Code from [METRO_UAT1].dbo.[DPCODES] a
join [METRO_UAT1].dbo.[DPCODES] b on b.code = a.campaign
where a.FIELDNAM = 'sol' and b.FIELDNAM = 'campaign' and SUBSTRING(a.CODE,3,2) NOT LIKE '%[^0-9]%'
)xx
left join
[Migration_Metro].dbo.[Campaign_Load_Hiearchy1] c1
...
update  [Migration_Metro].dbo.[Campaign_Load_Hiearchy2] 
...
--use [Migration_Metro]
--Exec SF_BulkOps 'Insert:bulkapi,parallel,Batchsize(10000)', 'TSAC_MIGR', 'Campaign_Load_Hiearchy2'